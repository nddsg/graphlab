...
def getGraphLabCommand(path, filename, directed):
    return "/usr/bin/time -v ~/graphlab/release/toolkits/graph_analytics/approximate_diameter %s/%s --format snap" % (path, filename)
    ...
    for f in edgefactor:
        ...
        for b in batch:
            for d in ["0", "1"]:
                ...
                outfile = "resultsgraphlabpseudo/%s.out" % (filename)
                if isfile(outfile):
