...
struct vertex_data {
  ...
  bool isDead;
	bool sent;
  vertex_data(wiki_page_type type = 0, distance_type dist = std::numeric_limits<distance_type>::max(),isDead=false,sent = false) :
    dist(dist), type(type) { 		}

	void save(graphlab::oarchive& oarc) const { 
		oarc << dist << type << vid_set<<isDead; 
	}
	
	void load(graphlab::iarchive& iarc) { 
		iarc >> dist >> type >> vid_set>>sent; 
	} 
...
//		std::cout<<"scattering to "<<other.id()<<"\n";
    if(other.data().type==14 && vertex.data().type == 14) {//category to category 
      ...
      for_cat.collection.push_back(tp2); //updating distance
	   ...
	   context.signal(other,std::make_pair(for_cat,vertex.id()));
   }
      ...
      if(other.data().vid_set.find( temp1[i].vid) != other.data().vid_set.end()){// It exists in the neighbours
       triple tp2(temp1[i].vid,temp1[i].from_src+1,temp1[i].from_last_art+1);
		...
		context.signal(other,std::make_pair(for_art,vertex.id()));
  }
  ...
  context.signal(other,std::make_pair(for_cat1,vertex.id()));
